...
const usersServices = {
    findUserById: async (id: number) => {
        const [user]: [IUserSQL[], FieldPacket[]] = await pool.query(`SELECT id, firstName, lastName, email, role, dateCreated FROM users WHERE id = ? AND dateDeleted IS NULL`, [id])
        return user[0];
    },
    findUserByEmail: async (email: string) => {
        const [user]: [IUserSQL[], FieldPacket[]] = await pool.query(`SELECT id, email, password, role FROM users WHERE email = ? AND dateDeleted IS NULL`, [email]);
        return user[0];
    },
    getAllUsers: async () => {
        const [users]: [IUserSQL[], FieldPacket[]] = await pool.query('SELECT id, firstName, lastName, email, role, dateCreated FROM users;');
        return users;
    },
    ...
    updateUser: async (userToUpdate: IUser ): Promise<Boolean> => {
        ...
        const [result]: [ResultSetHeader, FieldPacket[]] = await pool.query(`UPDATE users SET ? WHERE id = ?`, [updatedData, id]);
